...
void MapUI::PrintMenu() {
  ...
  case '5':
  {
    ...
    auto start = std::chrono::high_resolution_clock::now();
    auto results = map.CycleDetection(subgraph, square,cycle_nodes);
    auto stop = std::chrono::high_resolution_clock::now();
      PlotPathandSquare(cycle_nodes,square);
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    menu = "*************************Results******************************\n";
    std::cout << menu;
    if (results == true)
...
void MapUI::PlotPathandSquare(std::vector<std::string> &location_ids, std::vector<double> &square) {
  std::string image_path = cv::samples::findFile("src/lib/map.png");
  cv::Mat img = cv::imread(image_path, cv::IMREAD_COLOR);
  // cv::resize(img, img, cv::Size(img.cols, img.rows));
...
void MapUI::DynamicPrintMenu() {
  ...
  case '4':
  {
    menu =
        ...
        "**************************************************************\n";
    y = ui.ScrollLongText(menu);
    menu = "Please input number of nodes:";
    y = ui.ScrollLongText(menu, 10, y);
    char input1[100];
    scanw("%s",input1);
    int num = std::stoi(input1);
    ...
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    PlotPath(results.second.back());
    clear();
    menu = "*************************Results******************************";
    y = ui.ScrollLongText(menu);
    y=ui.ScrollLongText("Time taken by function: " + std::to_string(results.first) + " miles",10,y);
    menu = "**************************************************************\n";
    y=ui.ScrollLongText(menu,10,y);
    y=ui.ScrollLongText("Time taken by function: " + std::to_string(duration.count()/1000) + " ms",10,y);
    y=ui.ScrollLongText("Press any keys to continue.",10,y);
  ...
  case '5':
  {
    ...
    scanw("%s",input1);
    std::vector<double> square;
    square.push_back(std::stod(input1));
    ...
    menu = "*************************Results******************************\n";
     y = ui.ScrollLongText(menu);
    if(results){
      y=ui.ScrollLongText("There is a cycle in the subgraph",10,y);
    }
    else{
  ...
  case '7':
  {
    ...
    menu = "Please input the location:";
    y = ui.ScrollLongText(menu, 10, y);
    char origin[100];
    scanw("%s",origin);
    ...
    char input1[100];
    scanw("%s",input1);
    int r = std::stoi(input1);

    menu = "Please input number k:";
    y = ui.ScrollLongText(menu, 10, y);
    scanw("%s",input1);
    int k = std::stoi(input1);
    ...
    y=ui.ScrollLongText(menu);
    for (auto x : result) { 
      y=ui.ScrollLongText(map.GetName(x),10,y);  
      cnt++;
    }
    menu = "**************************************************************\n";
    y=ui.ScrollLongText(menu,10,y);
